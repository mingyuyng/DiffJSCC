...
parser.add_argument("--jscc_weight", type=str, required=True)
...
swinir_weights = load_weight(args.jscc_weight)
...
for weight_name in scratch_weights.keys():
    ...
    if weight_name.startswith("control_"):
        ...
        target_model_weights = sd_weights
    elif weight_name.startswith("blip_model."):
        continue
    else:
...
torch.save(init_weights, args.output)
print("Done.")
