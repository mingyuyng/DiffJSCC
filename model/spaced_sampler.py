...
class SpacedSampler:
    ...
    def sample(
        ...
        # Initialize the noise or noisy image
        if x_T is None:
        ...
        if cond_text is not None:  # If we have text prompts
            cond = {
                "c_spatial": [self.model.apply_condition_encoder(cond_img)],
                "c_textual": [cond_text],
                "c_snr": [cond_snr]
            }
        else:  # If we don't have text prompts
            cond = {
                "c_spatial": [self.model.apply_condition_encoder(cond_img)],
                "c_textual": [self.model.get_learned_conditioning([positive_prompt] * b)],
                "c_snr": [cond_snr]
            }
